	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		switch($dest) {
			case 'I': {
				...
				if (ob_get_contents()) {
					$this->Error('Some data has already been output, can\'t send PDF file get_conte');
				}
				if (php_sapi_name() != 'cli') {
					// send output to a browser
					header('Content-Type: application/pdf');
					if (headers_sent()) {
						$this->Error('Some data has already been output to browser, can\'t send PDF file header');
					}
