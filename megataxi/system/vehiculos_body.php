...
<tr>
<td>
<?php if($_SESSION[datos]->perfil==1) { include('../forms/aplica_pago.form.php'); } ?>
</td>
	<td>
...
<table border="1" align="center">
  <tr class="tr_title">
    ...
    <td width="140">Frecuencia</td>				
    <td width="140">Suspencion</td>
...
$class='';
$estado_frecuencia="PAGO";
$estado_suspencion="En funcionamiento";
$mensaje="";
if($row_vehiculos[frecuencia]==0){
	$mensaje="Suspendido por pago";
	$class='bgcolor="#F99D9D"';
	$estado_frecuencia="NO PAGO";
}
if($row_vehiculos[suspencion]==0){
	$mensaje="Suspendido por ".$row_suspension[nombre]." Durante ".$row_suspension[duracion]." Horas, desde las ".$row_suspension[fecha]." hasta las ".$vence;
	$class='bgcolor="#F99D9D"';
	$estado_suspencion="SUSPENDIDO";
}

?>

    <tr <?php echo $class; ?>  title="<?php echo $mensaje;?>" >
	...
	<?php } ?>
      ...
      <td><?php echo $estado_frecuencia; ?></td>
      <td><?php echo $estado_suspencion; ?></td>
	...
	<td>
<form action="../process/vehiculo.process.php" method="post" target="IFrameProcess" id="SuspenderVehiculo<?php echo $placa; ?>">
<input type="hidden" name="modo" value="<?php if($row_vehiculos[suspencion]==1) echo "suspender"; else echo "quitarSuspension"; ?>" />
<input type="hidden" name="placa" value="<?php echo $row_vehiculos[placa]; ?>" />
<?php if($_SESSION[datos]->perfil==1){  ?>
<input type="button" value="<?php if($row_vehiculos[suspencion]==1) echo "Suspender"; else echo "Quitar Suspension"; ?>" <?php if($row_vehiculos['suspencion']==1){ ?> onclick="abrir('/megataxi/forms/suspension.form.php','','&<?php echo $_SERVER['QUERY_STRING']; ?>&vehiculo=<?php echo $row_vehiculos[placa]; ?>','Edicion');ver_div_edit();" <?php }else{ ?> onclick="enviar('SuspenderVehiculo<?php echo $row_vehiculos[placa]; ?>');" <?php } ?> />
<?php } ?>
</form>
</td>
