   ...
   ?>
    ...
    <tr id="servicios<?php echo $i; ?>" ondblclick="seleccionar_fila('servicios','<?php echo $i;  ?>');"<?php if($row_servicios[tipo_servicio]==1){ echo " style=\"color:#000000;\"";}else if($row_servicios[tipo_servicio]==2){  echo " style=\"color:#000C8F;\""; }else if($row_servicios[tipo_servicio]==3){ echo " style=\"color:#9F0006;\""; } ?>>
      <td><input name="servicio_id" type="hidden" id="servicio_id<?php echo $i;  ?>" value="<?php echo $row_servicios['servicio_id']; ?>" />
		<a href="../llamar.php?numero=<?php echo $row_servicios['telefono']; ?>" target="IFrameProcess"><?php echo $row_servicios['telefono']; ?>
      	</a>
      </td>
	  ...
	  <td>
		...
		<?php $row_reportado=mysql_fetch_assoc($result_Reportados); ?>
	    <input type="hidden" name="mega11[]" onclick="seleccionar_fila('servicios','<?php echo $i;  ?>');this.focus();" onfocus="this.select();" onblur="accion_objeto_blur('mega11','<?php echo $i; ?>');" onkeypress="accion_objeto(event,'mega11','<?php echo $i; ?>');" id="mega11<?php echo $i ?>" value="<?php echo $row_reportado[mega]; ?>" size="3" />
		<input type="hidden" id="megaHidden11<?php echo $i ?>" name="megaHidden11[]" value="<?php echo $row_reportado[mega]; ?>" />
		<?php $row_reportado=mysql_fetch_assoc($result_Reportados); ?>
	    <input type="hidden" name="mega12[]" onclick="seleccionar_fila('servicios','<?php echo $i;  ?>');this.focus();" onfocus="this.select();" onblur="accion_objeto_blur('mega12','<?php echo $i; ?>');" onkeypress="accion_objeto(event,'mega12','<?php echo $i; ?>');" id="mega12<?php echo $i ?>" value="<?php echo $row_reportado[mega]; ?>" size="3" />
		<input type="hidden" id="megaHidden12<?php echo $i ?>" name="megaHidden12[]" value="<?php echo $row_reportado[mega]; ?>" />
		<?php $row_reportado=mysql_fetch_assoc($result_Reportados); ?>
	    <input type="hidden" name="mega13[]" onclick="seleccionar_fila('servicios','<?php echo $i;  ?>');this.focus();" onfocus="this.select();" onblur="accion_objeto_blur('mega13','<?php echo $i; ?>');" onkeypress="accion_objeto(event,'mega13','<?php echo $i; ?>');" id="mega13<?php echo $i ?>" value="<?php echo $row_reportado[mega]; ?>" size="3" />
		<input type="hidden" id="megaHidden13<?php echo $i ?>" name="megaHidden13[]" value="<?php echo $row_reportado[mega]; ?>" />
		<?php $row_reportado=mysql_fetch_assoc($result_Reportados); ?>
	    <input type="hidden" name="mega14[]" onclick="seleccionar_fila('servicios','<?php echo $i;  ?>');this.focus();" onfocus="this.select();" onblur="accion_objeto_blur('mega14','<?php echo $i; ?>');" onkeypress="accion_objeto(event,'mega14','<?php echo $i; ?>');" id="mega14<?php echo $i ?>" value="<?php echo $row_reportado[mega]; ?>" size="3" />
		<input type="hidden" id="megaHidden14<?php echo $i ?>" name="megaHidden14[]" value="<?php echo $row_reportado[mega]; ?>" />
		<?php $row_reportado=mysql_fetch_assoc($result_Reportados); ?>
	    <input type="hidden" name="mega15[]" onclick="seleccionar_fila('servicios','<?php echo $i;  ?>');this.focus();" onfocus="this.select();" onblur="accion_objeto_blur('mega15','<?php echo $i; ?>');" onkeypress="accion_objeto(event,'mega15','<?php echo $i; ?>');" id="mega15<?php echo $i ?>" value="<?php echo $row_reportado[mega]; ?>" size="3" />
		...
		<td><form action="../process/servicio.process.php" method="post" target="IFrameProcess" id="CancelaServicio<?php echo $i; ?>"><input type="image" src="../images/cancelar.png" border="0" onclick="enviar('CancelaServicio<?php echo $i; ?>');" /><input type="hidden" name="servicio_id" value="<?php echo $row_servicios[servicio_id]?>" /><input type="hidden" name="modo" value="cancelar" /></form></td>
        <td>
