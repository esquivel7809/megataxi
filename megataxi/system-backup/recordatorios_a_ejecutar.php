...
<table border="0" cellspacing="0" class="tablita">
  ...
  <?php do { ?>
    <tr>
      <td>
    <a href="../llamar.php?numero=<?php echo $row_recordatorios['telefono']; ?>" target="IFrameProcess" title="Click para Llamar"><?php echo $row_recordatorios['telefono']; ?>
      </a>
      </td>
      ...
      <td><?php $array_hora=split(" ",$row_recordatorios['hora']);
	  			echo $array_hora[1];
				 ?></td>
      <td><form id="recordatorioEjecutado<?php echo $row_recordatorios['id_recordatorio']; ?>" action="../process/recordatorio.process.php" target="IFrameProcess" method="post"><div style="padding:0 2px;text-align:center;"><img src="../images/tick.png" style="cursor:pointer;" onClick="enviar('recordatorioEjecutado<?php echo $row_recordatorios['id_recordatorio']; ?>');"><input type="hidden" name="modo" value="cancelar"><input type="hidden" name="id_recordatorio" value="<?php echo $row_recordatorios[id_recordatorio]; ?>"></div></form></td>
