	...
	$("#carnetcomunicacion").autocomplete("<?=site_url('comunicaciones/data/consultacarnetcomunicacion/')?>", { 
		...
		parse: function(data) {
			return $.map(data, function(row) {
				return {
					data: row,
					value: row.idcarnetcomunicacion,
					result: row.idcarnetcomunicacion
				}
			});
		},
		width: 500,
    ...
    <form target="_blank" method="post" name="form_vehiculoconductor" id="form_vehiculoconductor" action="<?=site_url('imprimir/carnetcomun/index')?>">
        <div class="form">
            <div>
                ...
                <input type="text" name="carnetcomunicacion" id="carnetcomunicacion" required="required" title="Conductor" placeholder="Digite No. carnet, cedula o placa" value="<?php if(!empty($datocarnet)) echo $datocarnet?>"/>
            </div>
            <div>
                <label>Placa:</label>
