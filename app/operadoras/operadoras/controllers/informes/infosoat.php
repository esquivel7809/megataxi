...
class Infosoat extends CI_Controller {
	/**
    ...
    */
	...
	{
        $data['vista_contenido_mod']=array("informes/infosoat_view");
    ...
    */
    ...
    {
        $data['vista_contenido_mod']=array("informes/infosoat_res_view");
        $titulo="";
        ...
        $condfecha=" AND s.fechafinal>='".$fechainicial."' AND s.fechafinal<='".$fechafinal."'";
        ...
        if($valoridaseguradora!="")
        {
        	...
        	$condicionaseguradora=" AND s.idaseguradora IN (".stripslashes($valoridaseguradora).")";
        }
        if($valoridvehiculo!="")
        {
        ...
        /**** Campos a Consultar *******/
        $sqlselect="s.numerosoat, s.fechafinal, a.nombreaseguradora, v.idvehiculo, v.placa,
                    m.nombremodelo, tv.nombretipovehiculo, mv.nombremarcavehiculo, v.asociado,
                    v.empresa, v.comunicacion, v.activo, c.nombrecompleto, c.telefonofijo, c.celular ";
        
        /***** Relacion de Tablas *****/
        $sqlfrom="FROM soat AS s 
                LEFT JOIN aseguradora AS a ON a.idaseguradora=s.idaseguradora
                LEFT JOIN vehiculo AS v ON v.idvehiculo=s.idvehiculo
        ...
        /***** Consulta completa********/
        $sqlconsulta="SELECT ".$sqlselect.$sqlfrom." WHERE".$condactivo.$condasociado.$condrodamiento.$condcomunicacion.$sqlcondicion. " AND s.activo='1'";
		...
		if(isset($arraypost["Html"])){
			...
			*/
			...
			$data['nombrearchivo']="informe_de_soat_".date('YmdHis').".xls";
			$this->load->view("include/plantilla4",$data);           
		}
    }
