...
class Infotarjetaoperacion extends CI_Controller {
	...
	public function __construct()
	{
        ...
        if(!empty($login))
        {
            if(empty($this->arrayRequest['idsubmoduloactual']) && !isset($this->arrayRequest['idinfotarjetaoperacion']))
            {
                terminar_session();
            }
            else
    ...
    /**
    * Muestra el formulario para el informe de Tarjetas de Operacion
    *
    * @access public
    * @param string
    * @return string
    */
	...
	{
        $data['vista_contenido_mod']=array("informes/infotarjetaoperacion_view");
        
        //Array que define los titulos que llevaran los criterios
        $Titulo=array('Placa','Modelo','Tipo', 'Marca');
        //Array que define los campos id' de los criterios en laBD
        $CamposId=array('idvehiculo','idmodelo','idtipovehiculo','idmarcavehiculo');
        //Array que define la ruta para la busqueda
        $rutaBusqueda=array(site_url('informes/data/consultarvehiculo'),site_url('informes/data/consultarmodelo'),site_url('informes/data/consultartipovehiculo'),site_url('informes/data/consultarmarcavehiculo'));
    ...
    /**
    * Muestra el resultado para el informe de Tarjetas de Operacion
    *
    * @access public
    * @param string
    * @return string
    */
    ...
    {
        $data['vista_contenido_mod']=array("informes/infotarjetaoperacion_res_view");
        $titulo="";
        ...
        $condfecha=" AND top.fechafinal>='".$fechainicial."' AND top.fechafinal<='".$fechafinal."'";
        ...
        /**** Campos a Consultar *******/
        $sqlselect="top.numerotarjetaoperacion, top.fechafinal, v.idvehiculo, v.placa,
                    m.nombremodelo, tv.nombretipovehiculo, mv.nombremarcavehiculo, v.asociado,
                    v.empresa, v.comunicacion, v.activo ";
        
        /***** Relacion de Tablas *****/
        $sqlfrom="FROM tarjetaoperacion AS top 
                LEFT JOIN vehiculo AS v ON v.idvehiculo=top.idvehiculo
        ...
        /***** Consulta completa********/
        $sqlconsulta="SELECT ".$sqlselect.$sqlfrom." WHERE".$condactivo.$condasociado.$condrodamiento.$condcomunicacion.$sqlcondicion. " AND top.activo='1'";
        
        /***** Datos que pasan a la vista *****/            
        $data['result']=$this->db->query($sqlconsulta);
        $data['arraysino']=array('1'=>'SI','0'=>'NO');
        $data['tituloInfo']=$this->informes->encabezadoInforme('INFORME DE TARJETAS DE OPERACIÓN',$titulo,$fechainicial,$fechafinal,true);;
		...
		if(isset($arraypost["Html"])){
			...
			/*
			$data['imprimir']=false;
			$nombrearchivo="informe_de_tarjetas_de_operacion_".date('YmdHis').".pdf";
			...
			*/
			...
			$data['nombrearchivo']="informe_de_tarjetas_de_operacion_".date('YmdHis').".xls";
			$this->load->view("include/plantilla4",$data);           
		}
    }
    
}
/* End of file infotarjetaoperacion.php */
/* Location: ./application/controllers/informes/infotarjetaoperacion.php */