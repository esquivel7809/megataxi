...
$(document).on("click", ".abrirAjax", function () {
	var id = $(this).data('id');
...
function guardar_refrendacion(id, obj, idform){
	if(id!=""){
		if(confirm("Esta Seguro Que Desea Refrendar")){
			//id = $(this).attr("id");
			$.ajax({
				url: base_url +'/conductores/refrendacion/registro',
				type: "POST",
				data: { id_puesto : id, Guardar : "Guardar" },
				success: function(data) {
					if(data!=""){
						$('#'+obj.id).remove();
						campo = '<input type="hidden" name="idrefrendacion" value="'+data+'"  /><input type="submit" id="imprimir_'+idform+'" name="imprimir_'+idform+'" value="Imprimir" />';
						$("#"+idform).append(campo);
