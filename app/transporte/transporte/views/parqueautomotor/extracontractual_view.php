<script>
	$(document).ready(function(){
		$('#form_extracontractual').html5form({
...
<div class="titulo-modulo">Registro de Seg. Extracontractual</div>
<div class="contenido-modulo border border-radius border-shadow">
    <form method="post" name="form_extracontractual" id="form_extracontractual" action="<?=site_url('parqueautomotor/extracontractual/registro')?>">
        <input type="hidden" name="idextracontractual" id="idextracontractual" value="<?php if(!empty($datoidextracontractual)) echo $datoidextracontractual?>" />
        <div class="form">
            <div>
                <label>Placa:</label>
            ...
            <?php echo $this->load->view('parqueautomotor/extracontractualform_view'); ?>
            <div>
                <label>Activo:</label>
