...
class Refrendacion_model extends CI_Model {
	/**
	 ...
	 */
		...
		/**
		 * Devuelve los datos de los vehiculos asociados para refrendacion a un conductor
		 * vc activo
		 *
		 ...
		 * @return	html
		 */
		public function cons_vehi_asoc_cond($idconductor)
		{
			if(!empty($idconductor)){
			
			$sql="SELECT vc.idvehiculoconductor, vc.idvehiculo, v.placa , s.numerosoat, s.fechainicial AS soat_fechainicial, s.fechafinal AS soat_fechafinal,
						 ...
						 rf.idrefrendacion, rf.fecharefrendacion, v.activo, mv.nombremarcavehiculo, v.numerointerno,
						 lc.numerolicenciaconductor, lc.fechavencimiento, cat.nombrecategoria
				  ...
				  LEFT JOIN soat as s ON s.idvehiculo = vc.idvehiculo AND s.activo='1'
				  LEFT JOIN contractual as c ON c.idvehiculo = vc.idvehiculo AND c.activo='1'
				  LEFT JOIN extracontractual as ec ON ec.idvehiculo = vc.idvehiculo AND ec.activo='1' 
				  LEFT JOIN revision as r ON r.idvehiculo = vc.idvehiculo AND r.activo='1'
				  ...
				  LEFT JOIN licenciaconductor AS lc ON lc.idconductor=vc.idconductor AND lc.activo='1'
				  LEFT JOIN categoria AS cat ON cat.idcategoria=lc.idcategoria			  
				  WHERE  (vc.idconductor = '".$idconductor."') AND vc.idempresa='".$this->idempresa."' AND vc.activo='1'
				  GROUP BY v.placa
				  LIMIT 0 , 20";
		...
		public function cons_dat_carnetcomunicacion($idcarnetcomunicacion)
		{
			if(!empty($idcarnetcomunicacion))
			{
				$sql="SELECT vr.idvehiculoradiotelefono, v.placa, rt.serieradiotelefono, mv.nombremarcavehiculo,
					  ...
					  INNER JOIN vehiculopropietario AS vp ON vp.idvehiculopropietario = cm.idvehiculopropietario
					  INNER JOIN conductor as con ON con.idconductor = vp.idconductor
