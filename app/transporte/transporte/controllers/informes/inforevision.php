...
class Inforevision extends CI_Controller {
	...
	public function __construct()
	{
        ...
        if(!empty($login))
        {
            if(empty($this->arrayRequest['idsubmoduloactual']) && !isset($this->arrayRequest['idinforevision']))
            {
                terminar_session();
            }
            else
    ...
    /**
    * Muestra el formulario para el informe de Revision Tecnicomecanica
    *
    * @access public
    * @param string
    * @return string
    */
	...
	{
        $data['vista_contenido_mod']=array("informes/inforevision_view");
        
        //Array que define los titulos que llevaran los criterios
        $Titulo=array('CDA','Placa','Modelo','Tipo', 'Marca');
        //Array que define los campos id' de los criterios en laBD
        $CamposId=array('idcda','idvehiculo','idmodelo','idtipovehiculo','idmarcavehiculo');
        //Array que define la ruta para la busqueda
        $rutaBusqueda=array(site_url('informes/data/consultarcda'),site_url('informes/data/consultarvehiculo'),site_url('informes/data/consultarmodelo'),site_url('informes/data/consultartipovehiculo'),site_url('informes/data/consultarmarcavehiculo'));
        //Array que define otras condiciones
        $otrasCondiciones=array('codigocda','','','','');
    ...
    /**
    * Muestra el resultado para el informe de Revision Tecnicomecanica
    *
    * @access public
    * @param string
    * @return string
    */
    ...
    {
        $data['vista_contenido_mod']=array("informes/inforevision_res_view");
        $titulo="";
        $arraypost=$this->input->post();
        $arrayidcda             =   $arraypost['idcda'];
		...
		$valoridcda="";
		if(is_array($arrayidcda) && !empty($arrayidcda))
		{
			foreach($arrayidcda as $value)
			{
				$valoridcda.=(empty($valoridcda))? "'".$value."'" :  ",'".$value."'" ; 
			}
		}
        $valoridvehiculo="";
        ...
        $condfecha=" AND r.fechafinal>='".$fechainicial."' AND r.fechafinal<='".$fechafinal."'";
        ...
        if($valoridcda!="")
        {
        	$titulo.=$this->informes->tituloFiltro($valoridcda,'cda',array('idcda'),'nombrecda','C.D.A.');
        	$condicionaseguradora=" AND c.idcda IN (".stripslashes($valoridcda).")";
        }
        if($valoridvehiculo!="")
        {
        ...
        /**** Campos a Consultar *******/
        $sqlselect="r.numerorevision, r.fechafinal, c.nombrecda, v.idvehiculo, v.placa,
                    m.nombremodelo, tv.nombretipovehiculo, mv.nombremarcavehiculo, v.asociado,
                    v.empresa, v.comunicacion, v.activo, cond.nombrecompleto, cond.telefonofijo, cond.celular ";
        
        /***** Relacion de Tablas *****/
        $sqlfrom="FROM revision AS r 
                LEFT JOIN cda AS c ON c.idcda=r.idcda
                LEFT JOIN vehiculo AS v ON v.idvehiculo=r.idvehiculo
				...
				LEFT JOIN vehiculopropietario AS vp ON vp.idvehiculo=v.idvehiculo
				LEFT JOIN conductor AS cond ON cond.idconductor=vp.idconductor
                ";
        ...
        /***** Consulta completa********/
        $sqlconsulta="SELECT ".$sqlselect.$sqlfrom." WHERE".$condactivo.$condasociado.$condrodamiento.$condcomunicacion.$sqlcondicion. " AND r.activo='1'";
        
        /***** Datos que pasan a la vista *****/            
        $data['result']=$this->db->query($sqlconsulta);
        $data['arraysino']=array('1'=>'SI','0'=>'NO');
        $data['tituloInfo']=$this->informes->encabezadoInforme('INFORME DE SOAT',$titulo,$fechainicial,$fechafinal,true);;
		...
		if(isset($arraypost["Html"])){
			...
			/*
			$data['imprimir']=false;
			$nombrearchivo="informe_de_revision_tecnico_mecanica_".date('YmdHis').".pdf";
			...
			*/
			...
			$data['nombrearchivo']="informe_de_revision_tecnico_mecanica_".date('YmdHis').".xls";
			$this->load->view("include/plantilla4",$data);           
		}
    }
    
}
/* End of file inforevision.php */
/* Location: ./application/controllers/informes/inforevision.php */