...
class Infoextracontractual extends CI_Controller {
	...
	public function __construct()
	{
        ...
        if(!empty($login))
        {
            if(empty($this->arrayRequest['idsubmoduloactual']) && !isset($this->arrayRequest['idinfoextracontractual']))
            {
                terminar_session();
            }
            else
    ...
    /**
    * Muestra el formulario para el informe de seguro Extra-Contractual
    *
    * @access public
    * @param string
    * @return string
    */
	...
	{
        $data['vista_contenido_mod']=array("informes/infoextracontractual_view");
    ...
    /**
    * Muestra el resultado para el informe de seguro Extra-Contractual
    *
    * @access public
    * @param string
    * @return string
    */
    ...
    {
        $data['vista_contenido_mod']=array("informes/infoextracontractual_res_view");
        $titulo="";
        ...
        $condfecha=" AND ec.fechafinal>='".$fechainicial."' AND ec.fechafinal<='".$fechafinal."'";
        ...
        if($valoridaseguradora!="")
        {
        	...
        	$condicionaseguradora=" AND ec.idaseguradora IN (".stripslashes($valoridaseguradora).")";
        }
        if($valoridvehiculo!="")
        {
        ...
        /**** Campos a Consultar *******/
        $sqlselect="ec.numeroextracontractual, ec.fechafinal, a.nombreaseguradora, v.idvehiculo, v.placa,
                    m.nombremodelo, tv.nombretipovehiculo, mv.nombremarcavehiculo, v.asociado,
                    v.empresa, v.comunicacion, v.activo ";
        
        /***** Relacion de Tablas *****/
        $sqlfrom="FROM extracontractual AS ec 
                LEFT JOIN aseguradora AS a ON a.idaseguradora=ec.idaseguradora
                LEFT JOIN vehiculo AS v ON v.idvehiculo=ec.idvehiculo
        ...
        /***** Consulta completa********/
        $sqlconsulta="SELECT ".$sqlselect.$sqlfrom." WHERE".$condactivo.$condasociado.$condrodamiento.$condcomunicacion.$sqlcondicion. " AND ec.activo='1'";
        
        /***** Datos que pasan a la vista *****/            
        $data['result']=$this->db->query($sqlconsulta);
        $data['arraysino']=array('1'=>'SI','0'=>'NO');
        $data['tituloInfo']=$this->informes->encabezadoInforme('INFORME DE SEGUROS EXTRACONTRACTUALES',$titulo,$fechainicial,$fechafinal,true);;
		...
		if(isset($arraypost["Html"])){
			...
			/*
			$data['imprimir']=false;
			$nombrearchivo="informe_de_seguros_extra_contractuales_".date('YmdHis').".pdf";
			...
			*/
			...
			$data['nombrearchivo']="informe_de_seguros_extra_contractuales_".date('YmdHis').".xls";
			$this->load->view("include/plantilla4",$data);           
		}
    }
    
}
/* End of file infoextracontractual.php */
/* Location: ./application/controllers/informes/infoextracontractual.php */