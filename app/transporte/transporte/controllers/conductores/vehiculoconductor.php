...
class Vehiculoconductor extends CI_Controller {
	...
	public function __construct()
	{
        ...
        if(!empty($login))
        {
            if(empty($this->arrayRequest['idsubmoduloactual']) && !isset($this->arrayRequest['idvehiculoconductor']))
            {
                //terminar_session();
            }
            else
    ...
    /**
    * Muestra el formulario de registro de las tarjetas de control
    *
    * @access public
    * @param string
    * @return string
    */
	public function index($array_datos)
	{
        ...
        $data['vista_contenido_mod']=array("conductores/vehiculoconductor_view");
    ...
    /**
    * Guarda la informacion de registro de las tarjetas de control
    *
    * @access public
    * @param string
    * @return string
    */
    public function registro()
    {
        ...
        $this->tabla->settabla('vehiculoconductor');
        ...
        $campos['numerotarjeta']    =   $arraypost['numerotarjeta'];
		//por defecto se  pone activo 
		$campos['activo']=1;
		
        if($arraypost["Guardar"]=="Guardar")
        {
			//si existen se inactivan los vehiculos asociados a ese conductor
			$campos_activo['activo']    =   0;
			$condicion_activo = array('idconductor' => $arraypost['idconductor'],'idvehiculo' => $arraypost['idvehiculo'], 'idempresa'=> $this->idempresa);
			$this->tabla->actualizarDatos($campos_activo,$condicion_activo);

            $campos['idvehiculoconductor']      =   $this->tabla->MaximoDato("idvehiculoconductor") + 1;
        ...
        {
            $condicion = array('idvehiculoconductor' => $arraypost['idvehiculoconductor']);
...
/* End of file vehiculoconductor.php */
/* Location: ./application/controllers/conductores/vehiculoconductor.php */