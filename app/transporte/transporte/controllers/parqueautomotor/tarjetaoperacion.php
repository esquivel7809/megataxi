...
class Tarjetaoperacion extends CI_Controller {
	/**
	...
	public function __construct()
	{
        ...
        if(!empty($login))
        {
            if(empty($this->arrayRequest['idsubmoduloactual']) && !isset($this->arrayRequest['idtarjetaoperacion']))
            {
                terminar_session();
            }
            else
    ...
    /**
    * Muestra el formulario para el registro de las tarjetas de operacion de los vehiculos
    *
    * @access public
    * @param string
    * @return string
    */
	public function index($array_datos)
	{
        ...
        $data['vista_contenido_mod']=array("parqueautomotor/tarjetaoperacion_view");
        $jsbuscar = "buscaRegistro('".site_url('parqueautomotor/data/buscartarjeta')."','divbuscar=busqueda&formulario=this.form.id',280,600,'Busqueda de Terjetas de Operacion','#busqueda')";
        ...
        if( !empty($this->arrayRequest['idtarjetaoperacion']) )
        {
			$this->tabla->settabla('tarjetaoperacion');
			$cond=array('idtarjetaoperacion'=>$this->arrayRequest['idtarjetaoperacion']);
            $datos= $this->tabla->consultarDatos("*",$cond,"","","","");
            $existeinfo = 1;
			$data['disabled']="disabled='disabled'";
            $data['datoidtarjetaoperacion']     = $datos[0]['idtarjetaoperacion'];
            ...
            $data['datonumerotarjetaoperacion']     = $datos[0]['numerotarjetaoperacion'];
    ...
    */
    public function registro()
    {
        ...
        if($arraypost["Guardar"]=="Guardar")
        {
            ...
            $condicionOtraTarjeta = array('numerotarjetaoperacion' => $arraypost['numerotarjetaoperacion'], 'idempresa'=> $this->idempresa);
        ...
        {
            $condicion = array('idtarjetaoperacion' => $arraypost['idtarjetaoperacion']);
...
/* End of file tarjetaoperacion.php */
/* Location: ./application/controllers/parqueautomotor/tarjetaoperacion.php */